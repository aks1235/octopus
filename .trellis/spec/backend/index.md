...
| [relay-stream-end.md](./relay-stream-end.md) | 流式结束判定契约:业务终态(finish_reason)与协议流结束([DONE])必须分离(否则丢 usage)、三协议终态事件是否流末的对照、成败判定不依赖 TCP 关闭方式、finish_reason 判定位置与空串防御、mutation 验证测试判别力 |
