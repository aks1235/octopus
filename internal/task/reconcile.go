...
// 作为 AutoSync=false 手改模型等遗漏路径的统一兜底。不禁用渠道(A 子任务置 Enabled=false 不等于删除)。
func GroupItemReconcileTask() {
	...
	log.Infof("reconcile deleted %d orphan group items, affected groups=%v", len(toDel), groupIDs)
}
