...
// B1 降级:上游无 keyID 概念,由 dev 的三段 channelID:keyID:modelName 改为两段。
...
// IsTripped 检查渠道是否处于熔断状态
// 返回 tripped=true 表示该渠道应被跳过,remaining 为剩余冷却时间。
...
func RecordFailure(channelID int, modelName string) {
	key := circuitKey(channelID, modelName)
